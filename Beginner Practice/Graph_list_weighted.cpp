...
// Node to represent a vertex in the adjacency list with weights
struct Node
{
    int data;
    int weight;
...
public:
    ...
    {
        Node *newNodeV = new Node(w, weight);
        newNodeV->next = adjList[v];
        adjList[v] = newNodeV;

        Node *newNodeW = new Node(v, weight);
        newNodeW->next = adjList[w];
        adjList[w] = newNodeW; // Uncomment this line for an undirected graph
    }
    ...
    void addVertex()
    {
        ...
        for (int i = 0; i < V; ++i)
        {
            ...
            while (current)
            {
                Node *newNode = new Node(current->data, current->weight);
    ...
    void printGraph()
    {
        for (int i = 0; i < V; ++i)
        {
            ...
            while (current)
            {
                std::cout << "(" << current->data << ", " << current->weight << ") ";
                current = current->next;
            }
            std::cout << std::endl;
        }
